...
const goToBook = (book) => {
  ...
  if (book.type === 'knowledge_card' || displayName.includes('知识卡')) {
    uni.navigateTo({
      url: `/pages/public-book-list/public-book-list?subject=politics&categoryId=${book.categoryId}&title=${encodeURIComponent(displayName)}`
    });
  } else {
    uni.navigateTo({
      url: `/pages/public-book-detail/public-book-detail?id=${book.id}&bookId=${book.id}`
    });
  }
};
