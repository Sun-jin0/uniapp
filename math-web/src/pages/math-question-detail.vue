<template>
  <view class="app-container">
    ...
    <!-- 顶部导航栏 -->
    <view class="nav-bar">
      <view class="back-btn" @tap="goToBookshelf">
        <SvgIcon name="back" size="44" fill="#fff" />
      </view>
      <text class="nav-title">{{ topNavTitle || '练习' }}</text>
      <view class="nav-right">
        <view class="nav-icon-btn" @tap="openSettingsModal">
          <SvgIcon name="settings" size="44" fill="#fff" />
        </view>
        <view class="nav-icon-btn" @tap="goToBookshelf">
          <SvgIcon name="home" size="44" fill="#fff" />
        </view>
      </view>
    </view>
    ...
    <aside class="sidebar" :class="{ 'open': sidebarOpen }">
      ...
      <div class="sidebar-content">
        ...
        <div v-else-if="hasChapters">
          <div v-for="chapter in sidebarChapters" :key="chapter.name" class="chapter-group">
            ...
            <div class="chapter-questions-container" :class="{ 'expanded': expandedChapters.includes(chapter.name) }">
              <div v-for="question in chapter.questions" :key="question.QuestionID" class="question-item">
        ...
        <div v-else class="no-chapter-list">
          <div v-for="sid in sortedBookQuestionSourceIDs" :key="sid" class="sidebar-question-card" :class="{ 'active': String(currentVisibleSourceId) === String(sid) }" @click="navigateToQuestion(sid); toggleSidebar(false);">
    ...
    <div class="sidebar-overlay" :class="{ 'active': sidebarOpen }" @tap="toggleSidebar(false)"></div>
      ...
      <div class="question-display-area" id="question-display-area">
        ...
        <view v-else-if="currentQuestionData" id="question-content">
          ...
          <view v-if="analysisExpanded" class="analysis-section-wrapper">
            <view v-if="currentQuestionData.second_request && currentQuestionData.second_request.length > 0">
              <view class="question-detail-header">
                <view class="question-detail-title">题目详解</view>
              </view>
              ...
              <!-- 考点和疑难点 -->
              <view v-if="showKnowledgePoints && groupedKnowledgePoints.考点 && groupedKnowledgePoints.考点.length > 0" class="knowledge-card kaodian">
                <view class="knowledge-card-header">
                  <SvgIcon name="dot" size="24" fill="var(--primary-color)" />
                  <text>考点</text>
                </view>
                <view class="knowledge-card-body">
                  <view v-for="(item, index) in groupedKnowledgePoints.考点" :key="'kao' + index" class="kp-item">
                    <!-- #ifdef H5 -->
                    <view class="kp-title" v-html="transformContextString(item.KPTitle || (item._question_code && item._question_code.KPTitle) || item.Title || item.KPName || '考点内容')"></view>
                    <!-- #endif -->
                    <!-- #ifdef MP-WEIXIN -->
                    <towxml class="kp-title" :nodes="parseTextWithLatexForMp(item.KPTitle || (item._question_code && item._question_code.KPTitle) || item.Title || item.KPName || '考点内容')"></towxml>
                    <!-- #endif -->
                    <!-- #ifdef H5 -->
                    <view v-if="item._question_code && item._question_code.KPContent" class="kp-content" v-html="transformContextString(item._question_code.KPContent)" :style="{ fontSize: (fontSize - 3) + 'px' }"></view>
                    <!-- #endif -->
                    <!-- #ifdef MP-WEIXIN -->
                    <towxml v-if="item._question_code && item._question_code.KPContent" class="kp-content" :nodes="parseTextWithLatexForMp(item._question_code.KPContent)"></towxml>
                    <!-- #endif -->
                  </view>
                </view>
              </view>

              <view v-if="showKnowledgePoints && groupedKnowledgePoints.疑难点 && groupedKnowledgePoints.疑难点.length > 0" class="knowledge-card yinandian">
                <view class="knowledge-card-header">
                  <SvgIcon name="dot" size="24" fill="#ff9500" />
                  <text>疑难点</text>
                </view>
                <view class="knowledge-card-body">
                  <view v-for="(item, index) in groupedKnowledgePoints.疑难点" :key="'yi' + index" class="kp-item">
                    <!-- #ifdef H5 -->
                    <view class="kp-title" v-html="transformContextString(item.KPTitle || (item._question_code && item._question_code.KPTitle) || item.Title || item.KPName || '疑难点内容')"></view>
                    <!-- #endif -->
                    <!-- #ifdef MP-WEIXIN -->
                    <towxml class="kp-title" :nodes="parseTextWithLatexForMp(item.KPTitle || (item._question_code && item._question_code.KPTitle) || item.Title || item.KPName || '疑难点内容')"></towxml>
                    <!-- #endif -->
                    <!-- #ifdef H5 -->
                    <view v-if="item._question_code && item._question_code.KPContent" class="kp-content" v-html="transformContextString(item._question_code.KPContent)" :style="{ fontSize: (fontSize - 3) + 'px' }"></view>
                    <!-- #endif -->
          ...
          <view v-if="showRelatedQuestions && currentQuestionData.third_request && currentQuestionData.third_request.length > 0" class="related-section">
            ...
            <view class="related-list-modern" v-show="relatedExpanded">
              <view v-for="(rel, index) in currentQuestionData.third_request" :key="'rel' + index" class="related-item-modern" @tap="navigateToQuestion(rel.RelatedQuestionID || rel.ID || rel.QuestionID, currentBookId)">
                ...
                <!-- #ifdef H5 -->
                <view class="related-content" v-if="rel.RelatedQuestionText || rel.QuestionText" v-html="transformContextString(rel.RelatedQuestionText || rel.QuestionText)" :style="{ fontSize: (fontSize - 3) + 'px' }"></view>
                <!-- #endif -->
                <!-- #ifdef MP-WEIXIN -->
                <towxml class="related-content" v-if="rel.RelatedQuestionText || rel.QuestionText" :nodes="parseTextWithLatexForMp(rel.RelatedQuestionText || rel.QuestionText)"></towxml>
                <!-- #endif -->
              </view>
            </view>
          </view>
    ...
    <view class="bottom-action-card">
      <view class="action-items">
        <view class="action-item" v-if="studyMode === 'test'">
        ...
        <view class="action-item" @click="answerSheetOpen = !answerSheetOpen" v-if="studyMode === 'test'">
          <SvgIcon name="answer-sheet" size="44" fill="#666" />
        </view>
        <view class="action-item" @click="toggleSidebar">
        ...
        <view class="curated-modal-body">
          ...
          <view class="create-category-section">
            <view v-if="!isCreatingCategory" class="create-btn-wrapper">
              <view class="create-bank-link" @tap="isCreatingCategory = true">+ 新建分类</view>
            </view>
...
// Towxml 解析结果缓存
// MP-WEIXIN only: questionTextNodes and optionNodesCache removed for H5
...
// 题目类型判断（移到最前面，因为 extractedOptions 依赖它）
...
// MP-WEIXIN only: watch for Towxml node updates removed for H5
...
const groupedKnowledgePoints = computed(() => {
  ...
  currentQuestionData.value.second_request.forEach(item => {
    const busType = item.BusType;
    if (busType === KAO_DIAN_BUS_TYPE || busType === YI_NAN_DIAN_BUS_TYPE) {
      if (!grouped[busType]) {
        grouped[busType] = [];
      }
      grouped[busType].push(item);
    }
  });
...
const handleSearch = async () => {
  ...
  closeSearchModal();
  navigateToQuestion(qid, currentBookId.value);
};
...
const fetchQuestionData = async (qid) => {
  console.log('fetchQuestionData called with qid:', qid);
  if (!qid) {
    console.log('qid is empty, returning');
    return;
  }
  ...
  try {
    console.log('Fetching question data from API...');
    const response = await request({
    ...
    if (response.data && response.data[qid]) {
      ...
      console.log('Question data loaded successfully:', qid);
      ...
      console.error('Response data is empty or missing qid:', response);
    ...
    console.log('fetchQuestionData finished, loadingQuestion set to false');
...
onMounted(() => {
  ...
  // 优先使用 window.location.search 获取参数，更可靠
  const urlParams = new URLSearchParams(window.location.search);
  const options = {
  ...
  console.log('Parsed options from URL:', options);
  ...
  console.log('Before fetchQuestionData - currentVisibleSourceId:', currentVisibleSourceId.value);
  if (currentVisibleSourceId.value) {
    console.log('Calling fetchQuestionData from onMounted');
    fetchQuestionData(currentVisibleSourceId.value);
    ...
    console.log('No currentVisibleSourceId, skipping fetchQuestionData');
  }
  ...
  window.addEventListener('popstate', (event) => {
    ...
    if (questionId && questionId !== currentVisibleSourceId.value) {
      currentVisibleSourceId.value = questionId;
