    ...
    <view class="scroll-content">
      ...
      <template v-else>
        ...
        <view class="menu-container">
          ...
          <!-- 管理员功能组 -->
          <view class="menu-group-card" v-if="isAdmin">
            <view class="menu-item-row" @click="goToAdmin">
              <view class="icon-box admin-bg">
                <view class="inner-icon admin"></view>
              </view>
              <text class="item-label">管理员后台</text>
              <view class="item-arrow"></view>
            </view>
          </view>
...
const checkLoginStatus = () => {
  ...
  // 检查是否为管理员 (role为1时是管理员)
  const role = uni.getStorageSync('role');
  isAdmin.value = role === 1 || role === '1';
  console.log('检查登录状态:', { token: !!token, role, isAdmin: isAdmin.value });
...
const loadUserInfo = async () => {
  try {
    ...
    if (res.code === 0) {
      ...
      // 更新角色和管理员状态 (role为1时是管理员)
      if (res.data.role !== undefined) {
        uni.setStorageSync('role', res.data.role);
        isAdmin.value = res.data.role === 1 || res.data.role === '1';
      }
    ...
    // 即使 API 失败，也尝试从本地存储读取角色 (role为1时是管理员)
    const role = uni.getStorageSync('role');
    isAdmin.value = role === 1 || role === '1';
    console.log('从本地存储读取角色:', { role, isAdmin: isAdmin.value });
  }
};
...
onShow(() => {
  // 直接检查管理员状态，确保显示正确 (role为1时是管理员)
  const role = uni.getStorageSync('role');
  isAdmin.value = role === 1 || role === '1';
  console.log('onShow 检查管理员状态:', { role, isAdmin: isAdmin.value });
  loadPageData();
});
...
.admin-bg { background: #fee2e2; }
.admin { background-color: #dc2626; mask-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 10.99h7c-.53 4.12-3.28 7.79-7 8.94V12H5V6.3l7-3.11v8.8z"/></svg>'); }
