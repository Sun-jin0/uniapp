...
const config = {
  ...
  tagStyle: {
    ...
    pre: 'font-family:monospace;white-space:pre-wrap;word-wrap:break-word;word-break:break-all;background-color:#f0f9f8;color:#000000;padding:8px 12px;border-radius:8rpx;border:1rpx solid #5FBDB5;line-height:1.4;',
...
Parser.prototype.onCloseTag = function (name) {
  ...
  if (i !== -1) {
    ...
    if (name === 'p') {
      // 将 p 标签改为 span 标签
      const pNode = this.stack[this.stack.length - 1]
      if (pNode && pNode.name === 'p') {
        pNode.name = 'span'
        // 给 span 添加块级样式
        if (!pNode.attrs.style) {
          pNode.attrs.style = ''
        }
        if (!pNode.attrs.style.includes('display')) {
          pNode.attrs.style += ';display:block'
        }
      }
    } else {
      // br 标签
...
Parser.prototype.popNode = function () {
  ...
  // #ifndef APP-PLUS-NVUE
  // 将 p 标签改为 span 标签，避免被 rich-text 渲染
  if (node.name === 'p') {
    node.name = 'span'
    // 给 span 添加块级样式
    if (!node.attrs.style) {
    ...
    if (!node.attrs.style.includes('display')) {
      node.attrs.style += ';display:block'
    }
    // 标记为行内元素，避免被拆分
    node.l = 'T'
  } else if (config.blockTags[node.name]) {
    node.name = 'div'
